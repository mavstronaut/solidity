...
bool TypeChecker::visit(ContractDefinition const& _contract)
{
	...
	if (function) {
		...
		if (function->isView())
			typeError(function->location(), "Constructor cannot be defined as view.");
		if (function->isPure())
		...
		if (function->name().empty())
		{
			...
			else
			{
				...
				if (fallbackFunction->isView())
					typeError(fallbackFunction->location(), "Fallback function cannot be declared view.");
...
bool TypeChecker::visit(FunctionDefinition const& _function)
{
	...
	if (_function.isPayable())
	{
		...
		if (_function.isView())
			typeError(_function.location(), "Functions cannot be view and payable at the same time.");
		if (_function.isPure())
			typeError(_function.location(), "Functions cannot be pure and payable at the same time.");
	}
	if (_function.isView() && _function.isPure())
			typeError(_function.location(), "Functions cannot be pure and view at the same time.");
